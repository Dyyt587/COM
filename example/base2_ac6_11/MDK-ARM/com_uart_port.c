...
#ifdef STM32H7
#define fifo_static_alloc(fifo,size,lock)do{\
		static uint8_t buf_#fifo[size];\
	fifo = kfifo_init(buf_#fifo,size,lock);\
}while(0)
#define fifo_static_alloc_DMA(fifo,size,lock)do{\
		static uint8_t buf_#fifo[size] __attribute__((section(".ARM.__at_0x30000000")));/*供dma使用*/\
	fifo = kfifo_init(buf_#fifo,size,lock);\
}while(0)
...
void com_user_init(void)
{
	...
	//COM_INIT(com1,4096,4096,4096,4096,com_type_uart);
