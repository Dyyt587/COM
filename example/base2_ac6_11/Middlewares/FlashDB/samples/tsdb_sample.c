...
void tsdb_sample(fdb_tsdb_t tsdb)
{
    ...
    { /* APPEND new TSL (time series log) */
        ...
        log_d("append the new status.temp (%d) and status.humi (%d)\n", status.temp, status.humi);
...
static bool query_cb(fdb_tsl_t tsl, void *arg)
{
    ...
    log_d("[query_cb] queried a TSL: time: %" __PRITS ", temp: %d, humi: %d\n", tsl->time, status.temp, status.humi);
...
static bool query_by_time_cb(fdb_tsl_t tsl, void *arg)
{
    ...
    fdb_blob_read((fdb_db_t) db, fdb_tsl_to_blob(tsl, fdb_blob_make(&blob, &status, sizeof(status))));
    log_d("[query_by_time_cb] queried a TSL: time: %" __PRITS ", temp: %d, humi: %d\n", tsl->time, status.temp, status.humi);
...
static bool set_status_cb(fdb_tsl_t tsl, void *arg)
{
    ...
    log_d("set the TSL (time %" __PRITS ") status from %d to %d\n", tsl->time, tsl->status, FDB_TSL_USER_STATUS1);
