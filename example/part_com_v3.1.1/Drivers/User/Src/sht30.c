...
static int sht30_send_cmd(SHT30_CMD cmd)
{
	...
	buf[0] = cmd >> 8;
	buf[1] = cmd & 0xFF;
// rec = HAL_I2C_Master_Transmit(&hi2c3, SHT30_ADDR_WR, (uint8_t*)buf, 2, 0xFFFF);
	com_send_addr(com2,SHT30_ADDR_WR,(uint8_t*)buf, 2,100);

					flag1=1;
//log_d("iic tx start");
//	HAL_I2C_Master_Transmit_IT(&hi2c3, SHT30_ADDR_WR, (uint8_t*)buf, 2);
//					while(flag1){
//						SCB_InvalidateDCache_by_Addr(&flag1,1);
...
static int sht30_single_shot_measurement(uint8_t *buf, uint8_t buf_size)
{
	...
	//com_receive_addr(com2,SHT30_ADDR_RD,SHT30_DATA_SIZE,0);
